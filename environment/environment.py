...
reward_list = [3, -0.3, 0.2, -0.2] #reward, wall, +, -
...
class GridEnvironment_2player:
    ...
    def set_position(self):
        reward1, reward2, wall = random.sample(range(1, self.grid_size*self.grid_size), 3)
        self.reward1_pos = [reward1 // self.grid_size, reward1 % self.grid_size]
        self.reward2_pos = [reward2 // self.grid_size, reward2 % self.grid_size]
        self.wall_pos = [wall // self.grid_size, wall % self.grid_size]
    ...
    def step(self, action, episode_ticks, agent_num):
        ...
        # 에이전트 이동
            ...
            if self.check_reward(agent_num):
                reward = reward_list[0] - (episode_ticks / 70) 
                if reward < 1e-16:
                    reward = 1e-16  # 리워드가 0일 때 작은 값으로 설정                
                return reward, self.agent1_done, old_pos
            #벽에 부딪힘. 
            ...
            #대부분의 이동
                ...
                old_pos_distance_1 = abs(old_pos[0] - self.reward1_pos[0]) + abs(old_pos[1] - self.reward1_pos[1])
        ...
        # 에이전트 이동
            ...
            if self.check_reward(agent_num):
                ...
                if reward < 1e-16:
                    reward = 1e-16  # 리워드가 0일 때 작은 값으로 설정                
                return reward, self.agent2_done, old_pos
            #벽에 부딪힘. 
            elif self.check_wall(agent_num):
                self.agent2_pos = old_pos 
                reward = reward_list[1]
                return reward, self.agent2_done, old_pos
            #대부분의 이동
            else:
